...
export function parseIntent(intent: string): IntentRequirements {
  ...
  return {
    ...
    shareholderRights: ownership || /\b(vote|voting|shareholder rights?)\b/.test(text),
...
export function checkIntent(intent: string, asset: AssetModel, product: ProductTruth, now = new Date()): IntentCheck {
  ...
  if (requirements.shareholderRights) {
  ...
  const ownershipMismatch = requirements.companyOwnership && (!product.rights.ownsUnderlying || !product.rights.shareholderRights);
